...
export const FuncGetTotalBalancePerUser = (data, id) => {
    const acountUser = data.find((user) => user.id === id);
    return acountUser === null || acountUser === void 0 ? void 0 : acountUser.transactions.reduce((acc, call) => {
        return call.type === DEPOSIT
            ? (acc += call.amount)
            : (acc -= call.amount);
    }, 0);
};
...
export const SumDeposit = (data) => {
    ...
    const totalTransaction = data.reduce((acc, call) => acc +
        call.transactions.filter((trans) => trans.type === DEPOSIT).length, 0);
    return totalTransaction > 0 ? totalDeposit / totalTransaction : 0;
};
export const FuncGetUsersWithAboveAverageTransactions = (data) => {
...
export const FuncSortUsersByActivityAndAge = (data) => {
...
export const FuncGetUserWithMostTransactions = (data, type) => {
...
export const FuncGetTop3HighestDeposits = (data) => {
    const totalDeposit = ExampleTotalTransaction(data, "deposit");
    const userMostDeposit = [...totalDeposit].sort((a, b) => b.transactions - a.transactions);
...
export const FuncGetTransactionHistoryForActiveUsers = (data) => {
    const userActive = ExampleCheckActivity(DataUsers, true);
    return [...userActive].map(user => {
